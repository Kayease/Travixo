...
const GalleryCard = ({ url, alt }: { url: string; alt: string }) => (
  <div className="relative w-full h-[280px] md:h-[320px] lg:h-[365px] rounded-xl overflow-hidden group cursor-pointer outline-none" tabIndex={0}>
    ...
    {/* Overlay with Instagram Icon - Static on touch devices, Slides down on Hover/Tap on desktop */}
    <div className="absolute inset-0 bg-black/40 z-10 w-full h-full translate-y-0 xl:-translate-y-full xl:group-hover:translate-y-0 group-focus:translate-y-0 transition-transform duration-500 ease-in-out flex items-center justify-center">
      <InstagramIcon />
    </div>
  </div>
);
