"use client";
import React from "react";

/**
 * BlogHeroSection Props Interface
 * Allows customization of title and subtitle for reusability
 */
interface BlogHeroSectionProps {
  /** Main title displayed in the hero section */
  title?: string;
  /** Subtitle/description text below the title */
  subtitle?: string;
}

/**
 * BlogHeroSection Component
 * 
 * A reusable hero banner section for the blog/journal pages.
 * Features a cream background with centered title and subtitle text.
 * 
 * Design Specifications (from Figma):
 * - Background: #FFF7E5 (warm cream)
 * - Title: Playfair Display, italic, 600 weight, 28px, #4B3621
 * - Subtitle: Poppins, 500 weight, 18px, #4B3621
 * - Desktop height: 285px
 * 
 * @param {BlogHeroSectionProps} props - Component props
 * @returns {JSX.Element} The rendered hero section
 */
export const BlogHeroSection: React.FC<BlogHeroSectionProps> = ({
  title = "Our Journal",
  subtitle = "Your Safari Hub: News, Tips, and Inspiration",
}) => {
  return (
    <section
      className="relative w-full py-16 md:py-20 lg:py-[98px]"
      style={{ backgroundColor: "#FFF7E5" }}
      aria-labelledby="blog-hero-title"
    >
      {/* Container with max-width for content alignment */}
      <div className="max-w-[1280px] mx-auto px-5 md:px-10 lg:px-20">
        {/* Centered Header Content */}
        <div className="text-center">
          {/* Main Title - Playfair Display Italic */}
          <h1
            id="blog-hero-title"
            className="font-display italic font-semibold text-[24px] md:text-[28px] leading-[37px] text-brand-brown mb-5 md:mb-6"
          >
            {title}
          </h1>

          {/* Subtitle - Poppins Medium */}
          <p className="font-body font-medium text-[16px] md:text-[18px] leading-[30px] text-brand-brown max-w-[360px] md:max-w-[415px] mx-auto">
            {subtitle}
          </p>
        </div>
      </div>
    </section>
  );
};

export default BlogHeroSection;
